...
# Power for Outcome 1's joint omnibus Wald against an RMS = 3 pp alternative.
...
# -----------------------------------------------------------------------------
# Sigma_Y under uniform K=4 (closed-form via enumeration of all 24 rankings)
# -----------------------------------------------------------------------------
...
# Note: cov() uses (n-1) denom; since we want population cov over the 24 equiprobable
# rankings, multiply by 23/24. Diagonal lands at exactly 0.25, off-diagonals at +-1/12.
...
cat("\n=== Asymptotic 80%-power n requirement ===\n")
print(results_df)
...
sim_power_phase1_diffuse <- function(n_per_arm, S = sim_S, B = sim_B) {
  ...
  for (s in seq_len(S)) {
    ...
    Sigma_hat <- cov(Y_T) / n_per_arm + cov(Y_C) / n_per_arm
    Sigma_pinv <- ginv(Sigma_hat)
    rk_S <- sum(svd(Sigma_hat)$d > 1e-8 * max(svd(Sigma_hat)$d))
    W <- as.numeric(t(tau_hat) %*% Sigma_pinv %*% tau_hat)
    p_val <- pchisq(W, df = rk_S, lower.tail = FALSE)
    if (p_val < alpha) reject <- reject + 1L
  }
  reject / S
}

# Find headline n_star
headline_n <- results_df$n_per_arm_80_wald[results_df$control_dgp == "uniform_K4" &
...
for (n_check in c(headline_n - 200, headline_n, headline_n + 200)) {
  if (n_check > 0) {
    sp <- sim_power_phase1_diffuse(n_check, S = sim_S, B = sim_B)
    cat(sprintf("  n_per_arm = %4d  empirical Wald power = %.3f\n", n_check, sp))
...
power_long <- power_curve_df %>%
  ...
  pivot_longer(cols = c(power_wald, power_tmax),
...
p_pow <- ggplot(power_long,
  ...
  labs(
    title    = "Asymptotic power for Outcome 1: joint Wald and T_max",
...
cat("\n=== DiD variant: empirical phase 2 pre/post ===\n")
...
p_pow_did <- ggplot(power_long_did,
  ...
  labs(
    title    = "Asymptotic power for Outcome 1 under a DiD design",
